...
export function constructLineObject(
    lineStrings: LineStringCoords[],
    ...
    const processedLineStrings: LineStringCoords[] =
        resolution !== null
            ? lineStrings.map((loop) =>
                  resampleLine(loop, resolution, ellipsoid ? "ellipsoid" : "grid"),
              )
            : lineStrings;
