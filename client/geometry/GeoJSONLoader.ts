...
function getBase<TType extends ParsedGeoJSONObject["type"], TData>(
    object: GeoJsonObject & { type: TType },
): BaseFields<TType, TData> {
    return {
        type: object.type,
...
function getLineObject(
    this: ParsedLineString | ParsedMultiLineString,
...
function getPolygonLineObject(
    this: ParsedPolygon | ParsedMultiPolygon,
    options: LineObjectOptions = {},
) {
...
export class GeoJSONLoader {
    ...
    parse(json: GeoJsonObject | string): ParsedGeoJSONResult {
        ...
        features.forEach((feature) => {
            ...
            feature.points = featureGeometries.filter((object) =>
                /Point/.test(object.type),
            ) as Array<ParsedPoint | ParsedMultiPoint>;
            feature.lines = featureGeometries.filter((object) => /Line/.test(object.type)) as Array<
                ParsedLineString | ParsedMultiLineString
            >;
            feature.polygons = featureGeometries.filter((object) =>
                /Polygon/.test(object.type),
            ) as Array<ParsedPolygon | ParsedMultiPolygon>;
        });

        return {
            features,
            geometries,
            points: geometries.filter((object) => /Point/.test(object.type)) as Array<
                ParsedPoint | ParsedMultiPoint
            >,
            lines: geometries.filter((object) => /Line/.test(object.type)) as Array<
                ParsedLineString | ParsedMultiLineString
            >,
            polygons: geometries.filter((object) => /Polygon/.test(object.type)) as Array<
                ParsedPolygon | ParsedMultiPolygon
            >,
        };
    }
    ...
    private parseObject(
        ...
        feature: ParsedFeature | null = null,
    ): ParsedGeoJSONObject {
        switch (object.type) {
            case "Point": {
            ...
            case "MultiPolygon": {
                ...
                return {
                    ...
                    data: coords,
                    dimension:
                        coords.length > 0 && coords[0].length > 0 && coords[0][0].length > 0
                            ? getDimension(coords[0][0][0])
                            : null,
                    getLineObject: getPolygonLineObject,
                    getMeshObject: getPolygonMeshObject,
            ...
            case "GeometryCollection": {
                ...
                const data = geometryCollection.geometries.map(
                    (geometry) =>
            ...
            case "Feature": {
                ...
                const parsedFeature: ParsedFeature = {
                    ...
                    properties: featureObject.properties as
                ...
                parsedFeature.data = featureObject.geometry
                    ? (this.parseObject(
                          featureObject.geometry as GeoJsonObject,
                          parsedFeature,
                      ) as ParsedGeometry)
                    : null;
            ...
            case "FeatureCollection": {
                ...
                const data = featureCollection.features.map(
                    (feat) => this.parseObject(feat as GeoJsonObject, null) as ParsedFeature,
                );
